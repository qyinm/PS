...
int minInit(int start, int end, int node) {
    ...
    return minTree[node] = min(minInit(start, mid, node * 2),
...
int maxInit(int start, int end, int node) {
    ...
    return maxTree[node] = max(maxInit(start, mid, node * 2),
...
int minFind(int start, int end, int left, int right, int node) {
    ...
    int mid = (start + end) / 2;
    return min(minFind(start, mid, left, right, node * 2),
               minFind(mid + 1, end, left, right, node * 2 + 1));
}
...
int maxFind(int start, int end, int left, int right, int node) {
    ...
    int mid = (start + end) / 2;
    return max(maxFind(start, mid, left, right, node * 2),
               maxFind(mid + 1, end, left, right, node * 2 + 1));
}
